...
Both are the *same* Compose Multiplatform codebase — `App()` picks the layout by screen width, not two separate apps.
...
shared/test/  EngineTest, FleetViewModelTest, FleetOverviewDataTest, SimulationDemoScriptTest.
...
See `TODO.md` for the section-by-section build checklist and `handing_over.md` for a detailed account of what's built, key design decisions, known assumptions to confirm, and what's left.
