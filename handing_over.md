...
Status as of this document: **Sections 1-12 of TODO.md are complete** (engine through localization/polish), **plus a full visual redesign pass** matching a reference mockup the user supplied (`WasteTrack UI — admin and driver.pdf`, in `~/Downloads`, not committed to the repo), and a new manual Admin/Driver mode switch + profile chip that isn't part of CLAUDE.md's spec. Sections 11 and 12 are genuinely web-verified with real browser screenshots (see below), not just compiled — real testing surfaced and fixed 6 genuine bugs across those two sections alone (see each section's writeup). Only Section 13 (demo prep) remains. A `README.md` now exists at the repo root. Read `CLAUDE.md` first — it's the full spec — then `TODO.md` for the section-by-section checklist. This file is the "what actually happened and what to watch for" layer on top of both.
...
1. `ZoneCanvas` computed its zoom bounding box from `zone.boundary + route`, so a 30-point route spanning kilometers made the 200m zone render as a barely-visible speck in a corner instead of the prominent "to-scale" inset both CLAUDE.md §13 and the mockup call for. Fixed: the projection now zooms to the zone itself plus a fixed margin (`ZONE_VIEW_MARGIN_FACTOR = 0.8`, so the zone fills roughly the middle third of the canvas). **Correction from this doc's earlier version**: this fix's own comment claimed route/position points outside the zoomed view "are simply clipped by the Canvas" — that turned out to be wrong (Compose's `Canvas` does not clip drawing to its own bounds by default) and caused a worse bug, found and fixed in Section 12 below. Don't trust that specific claim if you see it anywhere else; the real fix is `Modifier.clipToBounds()`.
...
### Localization and polish — Section 12
`shared/src/ui/Strings.kt`: a `Strings` data class with 22 fields (app name/tagline, 4 verdict labels, 7 nav labels, 3 Simulation buttons, 6 settings labels) — more than CLAUDE.md's "~15 or so is enough for the bonus," covering the most visible text. Reasons, banners, and chart copy deliberately stay English-only; that's a scope line, not an oversight — translating every string in the app wasn't the ask. `LocalStrings` (a `staticCompositionLocalOf`) is provided in `App.kt` based on `state.language`, and every screen/component that shows one of the 22 covered labels reads `LocalStrings.current` instead of a hardcoded string.

**The Twi strings need a caveat, twice over:**
1. **Translation accuracy is unverified.** These are a good-faith attempt, not reviewed by a native speaker. Confirm before using live in front of Ghanaian judges.
2. **Deliberately ASCII-only.** Proper Twi orthography uses ɛ (open e) and ɔ (open o) — e.g. "Ɛyɛ," "Sɔhwɛ." Real browser testing showed both characters render as tofu boxes (missing-glyph placeholders) in this environment's font. Since the actual presentation device's font support is unknown, the strings substitute the common e/o simplification long used in informal digital Twi (SMS, casual typing, where ɛ/ɔ aren't easily reachable on a keyboard) — e.g. "Eye," "Sohwe" — trading orthographic precision for guaranteed legibility. **If you confirm the demo device renders ɛ/ɔ correctly, restore them** in `Strings.kt` for accuracy; don't restore them blind.

**Three more real bugs found and fixed via live browser testing** of the dark/light consistency requirement (again: not from static review):
1. `ZoneCanvas`, `LoadOverTimeChart`, and `WeightGauge` all drew with hardcoded hex colors instead of the theme palette — invisible or jarring against a dark background. Fixed: all three now take a `palette: Palette` parameter and use `palette.compliantFg` / `palette.inProgressFg` / `palette.flaggedFg` etc. instead of fixed hex.
2. **Compose's `Canvas` paints its own opaque white backing rather than showing the parent `Surface`'s color through** — confirmed by literally seeing white boxes where a dark map/chart card should have been, in a real dark-mode screenshot. Fixed by having each canvas explicitly `drawRect(color = palette.surface)` as its first draw call.
3. **`Canvas` does not clip drawing to its own bounds by default.** `ZoneCanvas`'s zoomed-to-zone projection (from the Section 11 fix above) sends far-off route points to coordinates well outside the canvas's own size — with nothing to stop it, a route's distant tail rendered as a stray diagonal line across the *entire page*, crossing over unrelated cards and the sidebar. This is the one that would have been genuinely embarrassing live in front of judges. Fixed with `Modifier.clipToBounds()` on all Canvas-based components (`ZoneCanvas`, `CityWideMap` in `LiveMapScreen.kt`, `LoadOverTimeChart`, `WeightGauge`) — the last two weren't actually at risk by construction (their scale is derived from their own data's range, not an independently-zoomed range), but got the same defensive fix for consistency.

Also found `WeightGauge.kt` was **dead code** — built in Section 7, superseded by `LiveShiftScreen`'s own inline `LoadBar` during the visual redesign, and never reconnected to anything. Fixed it in place (palette-aware, like everything else) rather than deleting it, since `TODO.md` explicitly lists it as a completed Section 7 deliverable and deleting it would make that record inaccurate even though nothing currently calls it.

**Verification**: build, all 13 tests, and wasmJs compilation all green after every fix. Every fix above was re-verified live in the browser (not just re-compiled) before moving on — dark mode toggled, language toggled, Simulation panel and Live Map both re-screenshotted post-fix.
...
8. `3939ab7` — added a "How to demo this" section to README.md: run web/desktop from your own terminal rather than a sandboxed assistant process (which is why the Android emulator hung — no display/GPU access in that process), Android via a physical device as the more reliable fallback, and record a backup video ahead of time
9. *(pending as of this doc)* — Section 12 (localization + dark/light polish): `Strings.kt` (EN/TWI, ASCII-only Twi due to a real font-rendering gap), plus three Canvas dark-mode bugs found and fixed via live browser testing (hardcoded colors, opaque white backing, missing clip-to-bounds)
...
## What's next (Section 13)

1. **Section 13 — Demo prep.** Screen recording — web (`wasm-app` + a real browser) is the strongest option now that it's genuinely verified; consider it over jvm-app given the desktop-window display limitation above. Pitch rehearsal, backup video. The README's "How to demo this" section has the exact run commands and a suggested demo script.
2. **Have a native Twi speaker review `Strings.kt`'s `TWI_STRINGS`** before the pitch — both the translations themselves and whether the actual presentation device renders plain ASCII fine (it should; that's the whole point of the e/o substitution) versus whether it's worth restoring proper ɛ/ɔ orthography if that device's font supports it.
...
Per TODO.md's own fallback rule: if time runs out, stop at the end of the last fully-checked section rather than half-building the next one. Sections 1-12 are done, including the highest-value Simulation panel and real (not just compiled) verification of both remaining platforms' rendering — only demo prep is left.
